<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Nationalstate $nationalstate
 */
?>
<div class="row">
    <aside class="column">
        <div class="side-nav">
            <h4 class="heading"><?= __('Actions') ?></h4>
            <?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $nationalstate->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $nationalstate->id), 'class' => 'side-nav-item']
            ) ?>
            <?= $this->Html->link(__('List Nationalstates'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column-responsive column-80">
        <div class="nationalstates form content">
            <?= $this->Form->create($nationalstate) ?>
            <fieldset>
                <legend><?= __('Edit Nationalstate') ?></legend>
                <?php
                    echo $this->Form->control('name');
                    echo $this->Form->control('abbreviation');
                    echo $this->Form->control('dbpedia_url');
                ?>
            </fieldset>
            <?= $this->Form->button(__('Submit')) ?>
            <?= $this->Form->end() ?>
        </div>
    </div>
</div>

...
<div class="row">
    ...
    <div class="column-responsive column-80">
        <div class="manifestations form content">
            ...
            <p>Edit the manifestation of the <b><?php echo $letter->detailed_info; ?></b>.</p>
		            ...
		            <fieldset>
						...
						foreach($letter->senders as $tmp) {
							...
							if (isset($tmp->person)) {
								...
								echo '<input type="checkbox" class="form-check-input" id="signatures'.$tmp->person->id.'" name="manifestationSignatures[_senderIds][]" value="'.$tmp->id.'"';
									if (isset($signatures) && is_array($signatures)) {
										foreach($signatures as $signature) {
											if ($signature->sender_id == $tmp->id) {
												echo 'checked="checked"';
											}
										}
									}
									echo '/>';
									echo '<label class="form-check-label checkbox-label" for="signatures'.$tmp->person->id.'">'.$tmp->person->firstname.' '.$tmp->person->lastname.'</label>
								</div>';
							} else {
								echo '<div class="form-check">
									...
									echo '/>';
									echo '<label class="form-check-label checkbox-label" for="signatures'.$tmp->institution->id.'">'.$tmp->institution->name.'</label>
								</div>';
							}
							...
							foreach($letter->receivers as $tmp) {
								...
								if (isset($tmp->person)) {
									echo '<div class="form-check">
										...
										if (isset($annotations) && is_array($annotations)) {
											foreach($annotations as $annotation) {
												if ($annotation->receiver_id == $tmp->id) {
													echo 'checked="checked"';
												}
											}
										}
										echo '/>';
	            					echo '<label class="form-check-label checkbox-label" for="annotations-person'.$tmp->person->id.'">'.$tmp->person->firstname.' '.$tmp->person->lastname.'</label>
									</div>';
								} else {
									echo '<div class="form-check">
										...
										echo '/>';
	            					echo '<label class="form-check-label checkbox-label" for="annotations-instituion'.$tmp->institution->id.'">'.$tmp->institution->name.'</label>
									</div>';
								}
							...
							foreach($letter->senders as $tmp) {
								...
								if (isset($tmp->person)) {
									...
									<input type="checkbox" class="form-check-input" id="annotations-person'.$tmp->person->id.'" name="manifestationAnnotations[_senderIds][]" value="'.$tmp->id.'"';
										if (isset($annotations) && is_array($annotations)) {
											foreach($annotations as $annotation) {
												if ($annotation->sender_id == $tmp->id) {
													echo 'checked="checked"';
												}
											}
										}
			...
			<div class="row form-row">
				...
				<div class="row form-row">
					...
					echo $this->Form->control('archive_info', [
						...
						'class' => 'form-control',
						'label' => ['text' => 'Further details (URL, shelfmark, ...)']
					]);
					?>
